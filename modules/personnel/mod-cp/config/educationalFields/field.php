...
if(isset($_POST['title']) && !isset($_POST['id'])){
    ...
    $id=$database->id();
    langDefineSet(JK_LANG,'personnel_fields','id',$id,$_POST['title']);
...
if(isset($_POST['id'])){
    global $database;
    $group=$database->get('personnel_fields','*',[
            "id"=>$_POST['id'],
    ]);
    if(isset($group['id'])){
        $data['title']=langDefineGet(JK_LANG,'personnel_fields','id',$group['id']);
        echo \Joonika\Forms\field_hidden([
            "name"=>"id",
            "value"=>$_POST['id'],
        ]);
        if(isset($_POST['title'])){
            langDefineSet(JK_LANG,'personnel_fields','id',$group['id'],$_POST['title']);
            echo redirect_to_js();
        }
    }
}
