...
if(isset($_POST['title']) && !isset($_POST['id'])){
    global $Route;
    global $database;
    $database->insert('personnel_fields_cats',[
    ]);
    $id=$database->id();
    langDefineSet(JK_LANG,'personnel_fields_cats','id',$id,$_POST['title']);
...
if(isset($_POST['id'])){
    global $database;
    $group=$database->get('personnel_fields_cats','*',[
            "id"=>$_POST['id'],
    ]);
    if(isset($group['id'])){
        global $data;
        $data['title']=langDefineGet(JK_LANG,'personnel_fields_cats','id',$group['id']);
        echo \Joonika\Forms\field_hidden([
            "name"=>"id",
            "value"=>$_POST['id'],
        ]);
        if(isset($_POST['title'])){
            langDefineSet(JK_LANG,'personnel_fields_cats','id',$group['id'],$_POST['title']);
            echo redirect_to_js();
        }
    }
}
