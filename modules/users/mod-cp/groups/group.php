...
if(isset($_POST['title']) && !isset($_POST['id'])){
    global $Route;
    global $database;
    $database->insert('jk_groups',[
    ]);
    $id=$database->id();
    langDefineSet(JK_LANG,'jk_groups','id',$id,$_POST['title']);
...
if(isset($_POST['id'])){
    ...
    if(isset($group['id'])){
        global $data;
        $data['title']=langDefineGet(JK_LANG,'jk_groups','id',$group['id']);
        echo \Joonika\Forms\field_hidden([
            "name"=>"id",
            "value"=>$_POST['id'],
        ]);
        if(isset($_POST['title'])){
            langDefineSet(JK_LANG,'jk_groups','id',$group['id'],$_POST['title']);
            echo redirect_to_js();
        }
    }
}
