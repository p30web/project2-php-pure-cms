...
if(isset($_POST['title']) && !isset($_POST['id'])){
    global $Route;
    global $database;
    $database->insert('jk_users_locations_provinces',[
        "module"=>$countryID
    ]);
    $id=$database->id();
    langDefineSet(JK_LANG,'jk_users_locations_provinces','id',$id,$_POST['title']);
...
if(isset($_POST['id'])){
    global $database;
    $group=$database->get('jk_users_locations_provinces','*',[
            "id"=>$_POST['id'],
    ]);
    if(isset($group['id'])){
        $data['title']=langDefineGet(JK_LANG,'jk_users_locations_provinces','id',$group['id']);
        echo \Joonika\Forms\field_hidden([
            "name"=>"id",
            "value"=>$_POST['id'],
        ]);
        if(isset($_POST['title'])){
            langDefineSet(JK_LANG,'jk_users_locations_provinces','id',$group['id'],$_POST['title']);
            echo redirect_to_js();
        }
    }
}
