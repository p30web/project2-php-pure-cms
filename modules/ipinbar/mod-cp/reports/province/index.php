...
' . ajax_validate([
        "on" => "submit",
        "formID" => "report_form",
        "url" => JK_DOMAIN_LANG . 'cp/ipinbar/reports/province/getReport',
		...
		<div class="card-body">
			...
			<div class="container">
				<div class="row">
                    ...
                    $countries=$database->select('jk_users_locations_provinces',"id",[
                        "status"=>"active"
                    ]);
                    $cts=[];
                    if(sizeof($countries)>=1){
                        foreach ($countries as $country){
                            $cts[$country]=langDefineGet(JK_LANG,'jk_users_locations_provinces','id',$country);
                        }
                    }
                    echo div_start('col-md-9');
					echo \Joonika\Forms\field_select(
						[
							"name"=>"provinces[]",
							"id"=>"provinces",
							"title"=>__("provinces"),
							"attr"=>['placeholder'=>__("all provinces")],
							"ColType"=>"12,12",
							"multiple"=>true,
							"array"=>$cts,
						]
